...
export default function ProductSection() {
  ...
  const getCurrentIndex = (productId: number) =>
  ...
  const prevImage = (
    ...
    setCurrentImageIndexes((prev) => ({
      ...prev,
      [productId]:
        (getCurrentIndex(productId) - 1 + images.length) % images.length,
    }));
  };
  ...
  return (
    <>
      <section
        id="products"
        className="py-16 md:py-24 px-4 sm:px-6 md:px-10 bg-gradient-to-b from-gray-50 to-white"
      >
        <div className="max-w-7xl mx-auto">
                  ...
                  >
                    <div className="bg-white rounded-xl md:rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
                      <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
                        <img
                          src={`${IMG_URL}/${images[currentIndex]}`}
                        ...
                        {images.length > 1 && (
                          <div className="absolute inset-0 flex items-center justify-between px-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              onClick={(e) => prevImage(p.id, images, e)}
                              className="bg-white/90 p-1.5 md:p-2 rounded-full shadow-lg"
                            >
                              <ChevronLeft
                                size={14}
                                className="md:w-4 md:h-4 text-[#1e3a5f]"
                              />
                            </button>
                            <button
                              onClick={(e) => nextImage(p.id, images, e)}
                              className="bg-white/90 p-1.5 md:p-2 rounded-full shadow-lg"
                            >
                              <ChevronRight
                                size={14}
                      ...
                      <div className="p-4 md:p-6 space-y-2 md:space-y-3">
                        <h4 className="text-lg md:text-xl font-black text-[#1e3a5f] uppercase italic group-hover:text-cyan-600 transition-colors line-clamp-1">
                          {p.title}
                        </h4>
          ...
          <div className="mt-12 md:mt-16 flex justify-center px-4">
            ...
            >
              ...
              <ArrowRight
                size={14}
                className="md:w-4 md:h-4 group-hover:translate-x-2 transition-transform"
              />
            </button>
          </div>
        </div>
