...
export default function OrderingProcess() {
  ...
  return (
    <section
      id="carapemesanan"
      className="relative py-20 px-6 md:px-10 bg-gradient-to-b from-white to-gray-50 overflow-hidden"
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="relative">
          ...
          <div
            className="hidden lg:block absolute top-[72px] left-0 right-0 mx-auto w-[calc(100%-160px)] h-1 bg-gradient-to-r from-cyan-200 via-blue-200 to-purple-200 rounded-full"
            ...
            {steps.map((step, index) => (
              ...
              >
                ...
                <div className="relative mb-6 mt-4">
                  <div
                    className={`relative w-20 h-20 bg-gradient-to-br ${step.color} rounded-2xl flex items-center justify-center shadow-xl group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 z-10`}
                  >
                    <div className="text-white">{step.icon}</div>

                    {/* Glow Effect */}
                    <div
                      className={`absolute inset-0 bg-gradient-to-br ${step.color} rounded-2xl blur-xl opacity-0 group-hover:opacity-60 transition-opacity duration-300`}
                    />
                  </div>
                ...
                <div className="absolute -bottom-4 left-1/2 -translate-x-1/2">
                  <div
                    className={`w-2 h-2 bg-gradient-to-r ${step.color} rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
                  />
                </div>
              </div>
            ))}
