"use client";
import { Settings, Tag, Layers, Star, Zap, ShieldCheck } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {features.map((item, index) => (
            ...
            >
              ...
              <div
                className={`absolute inset-0 bg-gradient-to-br ${item.color} rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10 blur-xl`}
              />
              ...
              <div className="relative mb-6">
                <div
                  className={`inline-flex p-4 bg-gradient-to-br ${item.color} rounded-2xl text-white shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-300`}
                >
                  {item.icon}

                  {/* Glow Effect */}
                  <div
                    className={`absolute inset-0 bg-gradient-to-br ${item.color} rounded-2xl blur-lg opacity-0 group-hover:opacity-60 transition-opacity duration-300`}
                  />
                </div>

                {/* Stats Badge */}
                <div
                  className={`absolute -top-2 -right-2 px-3 py-1 bg-gradient-to-r ${item.color} text-white text-xs font-black rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
                >
                  {item.stats}
                </div>
              </div>
              ...
              {/* Bottom Accent Line */}
              <div
                className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r ${item.color} rounded-b-3xl transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left`}
              />
            </div>
          ))}
        </div>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-[#1e3a5f] via-[#2a4a6f] to-[#1e3a5f] p-12 md:p-16 text-center text-white shadow-2xl">
          ...
          <div className="absolute inset-0 overflow-hidden">
            ...
            <div
              className="absolute bottom-0 left-0 w-96 h-96 bg-blue-400/20 rounded-full blur-3xl animate-pulse-slow"
          ...
          <div className="relative z-10 space-y-6">
            ...
            <p className="text-cyan-100 max-w-2xl mx-auto text-base md:text-lg font-medium leading-relaxed">
              Konsultasi gratis untuk diskusi kebutuhan dan estimasi biaya. Tim
              kami siap membantu mewujudkan visi digital Anda.
            </p>

            <a
              href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "628157642627"}`}
              target="_blank"
